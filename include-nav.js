// include-nav.js  (pure JS)
(async function () {
  const mount = document.getElementById('navbar-include');
  if (!mount) return;

  // Fetch & inject navbar (cache-bust so iOS Chrome doesn't serve an old copy)
  const res = await fetch('navbar.html?v=final1', { cache: 'no-cache' });
  if (!res.ok) { console.error('navbar fetch failed', res.status); return; }
  mount.innerHTML = await res.text();

  // Element handles (support scoped tbc-* classes)
  ...
  // Click (desktop / non-touch). Ignore synthetic click after touch.
  ...
  // Close menu/dropdowns on outside tap, Esc, or breakpoint change
  ...
  // Keep Leaflet behind navbar (extra safety for map pages)
